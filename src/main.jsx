...
const STORAGE_KEY = 'RJP_STUDY_V2_DATA';
...
const demo = {
  ...
  profile: { name: 'Aluno 01', email: '', mode: 'Local / Offline' },
  subjects: [
    { id: crypto.randomUUID(), name: 'RMII', level: 'Ensino Superior', difficulty: 5, weeklyHours: 4, color: '#0f3b66', notes: 'Resistência dos Materiais II — disciplina de teste editável.' },
    { id: crypto.randomUUID(), name: 'Estruturas', level: 'Ensino Superior', difficulty: 4, weeklyHours: 3, color: '#b91c1c', notes: 'Análise de estruturas, exercícios e exames.' }
  ],
  evaluations: [],
  documents: [],
  exercises: [],
...
demo.evaluations = [
  { id: crypto.randomUUID(), subjectId: demo.subjects[0].id, type: 'Teste', title: 'Teste RMII', date: nextDate(14), matrix: 'Torção; Flexão; Plasticidade; Energia', weight: 'Alta', grade: '' }
];
demo.documents = [
  { id: crypto.randomUUID(), subjectId: demo.subjects[0].id, type: 'Exercícios Resolvidos', name: 'Exercícios RMII — exemplo', link: '', fileName: '', createdAt: new Date().toISOString() }
];
demo.exercises = [
  { id: crypto.randomUUID(), subjectId: demo.subjects[0].id, documentId: demo.documents[0].id, title: 'Exercício 5.1 — Torção', status: 'Pendente', minutes: 45, notes: '' },
  { id: crypto.randomUUID(), subjectId: demo.subjects[0].id, documentId: demo.documents[0].id, title: 'Exercício 5.6 — Flexão', status: 'Rever', minutes: 45, notes: '' },
  { id: crypto.randomUUID(), subjectId: demo.subjects[0].id, documentId: demo.documents[0].id, title: 'Exame 2024 — Questão 3', status: 'Não percebi', minutes: 60, notes: '' }
];
...
function Plan({plan}){return <div className="list">{plan.map(p=><div className="item plan" key={p.id}><h3>{p.subject}</h3><p>{p.day} • {p.minutes} min</p><p>{p.task}</p><span className="pill">{p.reason}</span></div>)}</div>}
...
function Stats({stats,data,subjectMap}){return <div className="grid">{Object.entries(stats.bySubject).map(([sid,s])=><Card key={sid}><h3>{subjectMap[sid]?.name}</h3><div className="percent small">{s.percent}%</div><p>Resolvidos: {s.done} / {s.total}</p><p>Rever: {s.review} • Não percebi: {s.bad}</p></Card>)}</div>}
